...
#### Handling other not standardized result values ####
  ...
  standard_records_percent <- ifelse(!is.na(n_records),
                                         ...
                                         NA)
  if (is.na(n_records)) {
    cat(paste0(green, success," ",blue, standard_records, reset, " result records were cleaned, classified, and standardized.\n"))
  }
  if (!is.na(n_records)) {
    cat(paste0(green, success," ",blue, standard_records, reset, " distinct results (",blue, standard_records_percent, "%", reset," of the total result records) were cleaned, classified, and standardized.\n"))
  }
  options(scipen = 999)
  end.time <- Sys.time()
